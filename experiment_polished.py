...
dataset['s_curve'] = make_s_curve(n_samples = 3200, random_state = 1)[0]

#swiss roll (2-dimensional)
dataset['swiss_roll'] = make_swiss_roll(n_samples = 3200, random_state = 1)[0]
...
strip = np.stack([x, y, z], axis = 1)
dataset['mobius_strip'] = strip.copy()
del theta, w, phi, r, x, y, z, strip
...
hollow = np.stack([x1, x2, x3], axis = 1)
dataset['hollow_sphere'] = hollow.copy()
del radius, azimuth, elevation, x1, x2, x3, hollow
...
solid = np.stack([x1, x2, x3], axis = 1)
dataset['solid_sphere'] = solid.copy()
del displacement, azimuth, elevation, x1, x2, x3, solid

for l in dataset.keys():
    value = dataset[l]
    xlim, ylim, zlim = limit(value)
    ...
    axes.set_ylim(*ylim)
    axes.set_zlim(*zlim)
    axes.view_init(azim = 75, elev = 20)
    plot = axes.scatter(value[:, 0], value[:, 1], value[:, 2], alpha = 0.3)
    fig.savefig('figures/training_sets/{name}.png'.format(name = l), dpi = 300)
del value, xlim, ylim, zlim, fig, axes, plot
...
for l in dataset.keys():
    begin = time.time()
    dimension, parse = estimator.estimate(dataset[l], return_parse = True)
    end = time.time()
    logger.info('elapsed time {name}: {lapse:.2f} (s)'.format(
        name = l,
        lapse = end - begin,
        ))
